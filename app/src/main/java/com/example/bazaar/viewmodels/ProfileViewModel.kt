...
class ProfileViewModel(val repository: Repository) : ViewModel() {
    var user: MutableLiveData<User> = MutableLiveData() //elmentem az adatot
    ...
    fun getProfile() {
        viewModelScope.launch {
            try {
                val result =
                    repository.getProfileInfo(MyApplication.username)
                user.value = result.user
                Log.d("xxx", "ProfileViewModel - #username:  ${result.user}")
            }catch(e: Exception){
